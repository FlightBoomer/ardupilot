...
void Copter::init_ardupilot()
{
    ...
    /// 自增加 光流控制初始化
    of_pos_control->set_dt(MAIN_LOOP_SECONDS);
#endif
...
void Copter::allocate_motors(void)
{
    ...
    // 自制惯导
    f_inav = new __optflow_inav_intf(ahrs);

    // 位置控制
    of_pos_control = new AC_PosControl(*ahrs_view, *f_inav, *motors, *attitude_control,
                                        g.p_alt_hold, g.p_vel_z, g.pid_accel_z,
                                        g.p_pos_xy, g.pi_vel_xy);
    if (of_pos_control == nullptr) {
        AP_HAL::panic("Unable to initialize optflow position control");
    }
