...
bool Copter::set_mode(control_mode_t mode, mode_reason_t reason)
{
    ...
    switch (mode) {
        ...
        case LIDARHOLD:
            success = lidarhold_init(ignore_checks);
            break;
...
{
    if (mode_has_manual_throttle(mode) || mode == LOITER || mode == ALT_HOLD || mode == POSHOLD || mode == DRIFT || mode == SPORT || mode == THROW || (arming_from_gcs && (mode == GUIDED || mode == GUIDED_NOGPS || mode == LIDARHOLD))) {
        return true;
    }
    return false;
}
...
void Copter::notify_flight_mode(control_mode_t mode)
{
    ...
    switch (mode) {
        ...
        case GUIDED_NOGPS:
            ...
            break;
        case LIDARHOLD:
            notify.set_flight_mode_str("LDAR");
        default:
